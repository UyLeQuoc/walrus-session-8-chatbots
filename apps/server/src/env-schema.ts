/**
 * What the server needs from its environment, apart from reading it, so the
 * schema can be checked against `.env.example` in CI without a real `.env`.
 */
import { operatorEnvSchema } from "@hippo/memory";
import { z } from "zod";
