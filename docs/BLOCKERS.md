...
- 2026-09-24 — **The production column for hide-one-memory.** The owner said yes in chat. `plan-push` showed exactly `ALTER TABLE "memory_index" ADD COLUMN "hidden_at" timestamp with time zone;`, no data loss, applied it, and a re-plan showed nothing left. Merged and deployed the same hour.
