...
- 2026-09-24 — **A yes to one new production column, so hide-one-memory can ship.** Branch `hide-one-memory` is built, tested and verified on mainnet (`docs/evidence/hide-one-memory-2026-09-24.md`). It needs `ALTER TABLE "memory_index" ADD COLUMN "hidden_at" timestamp with time zone;` — nullable, additive, nothing dropped. Schema pushes to production are the owner's call (`railway.toml`), and the server must not deploy before the column exists. Say yes and the agent runs `DATABASE_URL=<prod> pnpm --filter @hippo/db exec drizzle-kit push` showing the diff, merges, deploys and verifies. The branch also fixes a bare `/memory forget` erasing everything, and `forget` leaving pre-connect memories recallable for owned users.
