...
| 5 | Keep the story true | **done**: README, submission (the ticket table listed the old eleven, and repeated the `dropped_count` claim task 1 found wrong), RUNBOOK, PLAN, GOAL, AGENTS.md, this file. The CI guard now catches `.turbo` and `.env` files in any directory; four `.turbo` logs had been tracked since the scaffold |
